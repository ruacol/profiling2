...
GO
ALTER PROCEDURE [dbo].[PRF_SP_EventMerge_NHibernate]                                        
(                                        
 @ToKeepEventID INT,                                      
 @ToDeleteEventID INT,                                      
 @UserID NVARCHAR(450),                        
...
BEGIN                      
 ...
 BEGIN TRY                                        
  BEGIN TRANSACTION                                      
     ...
     UPDATE [dbo].[PRF_EventApproval] SET [EventID] = @ToKeepEventID WHERE [EventID] = @ToDeleteEventID

     UPDATE [dbo].[PRF_EventJhroCase] SET [EventID] = @ToKeepEventID WHERE [EventID] = @ToDeleteEventID
     ...
     DELETE FROM [dbo].[PRF_EventSource] 
     WHERE [EventID] = @ToDeleteEventID 
     AND [SourceID] IN (
        SELECT [SourceID]
        FROM [dbo].[PRF_EventSource] 
        WHERE [EventID] = @ToKeepEventID
     )   
     ...
     DELETE FROM [dbo].[PRF_EventViolation]
     WHERE [EventID] = @ToDeleteEventID
     AND [ViolationID] IN (
        SELECT [ViolationID]
        FROM [dbo].[PRF_EventViolation]
        WHERE [EventID] = @ToKeepEventID
     )
