...
namespace Profiling2.Infrastructure.Search.IndexWriters
{
    ...
    /// </summary>
    public sealed class ScreeningResponseIndexWriterSingleton
    {
        private static Directory directory = null;

        private static readonly Lazy<IndexWriter> lazy = new Lazy<IndexWriter>(() =>
                new IndexWriter(ScreeningResponseIndexWriterSingleton.Directory, new SimpleAnalyzer(), IndexWriter.MaxFieldLength.UNLIMITED)
            );

        public static IndexWriter Instance { get { return lazy.Value; } }

        private ScreeningResponseIndexWriterSingleton()
        {
        }
        ...
        public static Directory Directory
        {
            get
            {
                if (directory == null)
                {
                    string luceneIndexDirectory = System.IO.Path.Combine(ConfigurationManager.AppSettings["LuceneIndexDirectory"], ".\\LuceneScreeningResponseIndex");
